...
<?php
    ...
    $sql = "select * from vehicle where idNo=$idNo";
    $res = $db->query($sql);
    $row = $res->fetch_assoc();
    if (isset($_POST['submit']))
    {
        ...
        $status = $_POST['status'];$district = $_POST['district'];
        $q = "update vehicle 
        ...
        fuelType='$fuelType', status=$status, district='$district' where idNo=$idNo"; 
        $res1 = $db->query($q);
        if (!$res1)
            die($db->error);
        $db->close();
...
<?php require_once('header.php'); ?>
    <br>
    <h1 class="text-white text-center font-weight-bold bg-success" style="font-size: 55px;"> Edit Vehicle Data </h1>
    <div class="container"><br>
        <div class="col-lg-6 m-auto d-block">
            <form method="POST" onsubmit="return validation()" class="bg-light">
                ...
                <div class="form-group">
					<label for="district" class="font-weight-bold"> Mobile Number: </label>
